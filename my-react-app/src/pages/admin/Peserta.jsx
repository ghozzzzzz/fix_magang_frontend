...
export default function Komunitas() {
  ...
  const handleDeleteAnggota = async (id) => {
    ...
    try {
      ...
      // Refresh daftar anggota setelah menghapus
      if (currentKomunitas) {
        await fetchAnggota(currentKomunitas.id_komunitas);
      }
    } catch (err) {
      handleError(err);
    } finally {
            ...
            <div className="p-6">
              ...
              <div className="border-t border-gray-200 pt-4">
                <div className="flex justify-between items-center mb-3">
                  <h4 className="text-md font-medium text-gray-900">Daftar Anggota</h4>
                  <button 
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {anggota.map(agt => (
                          <tr key={agt.id}>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{agt.id}</td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              <a href={`tel:${agt.no_telepon}`}>{agt.no_telepon}</a>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              <button
                                onClick={() => navigate(`/anggota/${agt.id}/edit`)}
                                className="text-blue-600 hover:text-blue-900 mr-3"
                              >
                                Edit
                              </button>
                              <button
                                onClick={() => handleDeleteAnggota(agt.id)}
